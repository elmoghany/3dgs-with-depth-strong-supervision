    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if dataset.depth_dir and viewpoint_cam.gt_depth is not None:
            ...
            if D_pred is not None:
                M = M & (D_pred > 1e-3)                          # gate by visibility (rendered depth > 0)
            
            if M.sum() > 0:  # only if we have valid pixels
                err = (D_pred - D_gt)[M]                         # residuals over valid pixels
                
                # Robust loss (huber/l1/charb)
                if opt.depth_loss == "l1":
                    L_depth = err.abs().mean()
                elif opt.depth_loss == "charb":
                    eps = 1e-3
                    L_depth = torch.sqrt(err * err + eps * eps).mean()
                else:  # huber (default)
                    delta = 0.015  # ~1.5cm threshold; tighter for stronger supervision
                    ae = err.abs()
                    L_depth = torch.where(ae < delta, 0.5 * ae * ae / delta, ae - 0.5 * delta).mean()
                
                # Optional depth-gradient term (edge-aware geometry)
                if opt.depth_grad_weight > 0:
                    def _sobel(x):
                        kx = torch.tensor([[1,0,-1],[2,0,-2],[1,0,-1]], device=x.device, dtype=x.dtype).view(1,1,3,3)/4
                        ky = torch.tensor([[1,2,1],[0,0,0],[-1,-2,-1]], device=x.device, dtype=x.dtype).view(1,1,3,3)/4
                        x = x.unsqueeze(0).unsqueeze(0)
                        gx = torch.nn.functional.conv2d(x, kx, padding=1).squeeze()
                        gy = torch.nn.functional.conv2d(x, ky, padding=1).squeeze()
                        return gx, gy
                    
                    gx_p, gy_p = _sobel(D_pred)
                    gx_t, gy_t = _sobel(D_gt)
                    L_grad = ((gx_p - gx_t)[M].abs().mean() + (gy_p - gy_t)[M].abs().mean()) * opt.depth_grad_weight
                    L_depth = L_depth + L_grad
            ...
            # Fallback to old depth regularization if no GT depth
            Ll1depth_pure = 0.0
